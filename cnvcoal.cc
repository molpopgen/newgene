...
int main(int argc, char **argv)
{
  if ( argc != 11 )
    {
      cerr << "usage:\n"
	   << "cnvcoal nsam nsam2 nreps 4Nu 4Nr 4Nc nsites nsites_bw tlen seed\n"
	   << "where:\n"
	   << "\tnsam = sample size of ancestral gene\n"
	   << "\tnsam2 = sample size of copy-number variant\n"
	   ...
	   << "lines corresponding to the copy-number variant\n";
      exit(1);
    }
  unsigned argn=1;
  ...
  const double tau = 0.;     //time at which duplicate has fixed in the population, units of 4N generations
  ...
  vector<int> iconfig(1,nsam);
  iconfig.push_back(nsam2);
  vector<chromosome> isample = init_sample(iconfig,nsites);
  marginal imarg = init_marginal(nsam+nsam2);
  ...
  EVENT event;
  vector< pair<int,int> > __paired(nsam+nsam2,make_pair(0,0) );
  ...
  for(int i=0;i<nsam;++i)
	...
	{
	  __paired[i]=make_pair(i,-1);
	}
    }
  for(int i=nsam;i<nsam+nsam2;++i)
    {
      __paired[i]=make_pair(i,i-nsam);
    }
  ...
  while(howmany--)
    {
      ...
      config[0]=nsam;
      config[1]=nsam2;
      ENSAM = config[0]+config[1]-NLINKEDPAIRS;
      nAB = NLINKEDPAIRS;
	  ...
	  if(event == FIXATION)
	    {
	      ...
	      t = tau;
	      const double popfreq = gsl_ran_beta(r,1+nsam2,1+nsam-nsam2);
	      cerr << popfreq << '\n';
