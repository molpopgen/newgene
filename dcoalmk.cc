...
int main(int argc, char **argv)
{
  if ( argc != 12 )
    {
      cerr << "usage:\n"
	   << "dcoalmk nsam nreps 4Nu_s 4Nu_r 4Nr 4Nc nsites nsites_bw tlen tau seed\n"
	   << "where:\n"
	   << "\tnsam = sample size\n"
	   << "\tnreps = # of simulated samples to generate\n"
	   << "\t4Nu_s = theta, the population mutation rate at synonymous sites\n"
	   << "\t4Nr_r = rho, the population recombination rate at replacement sites\n"
	   ...
	   << "There are two ms outputs per replicate.  The first is for\n"
	   << "nonsynonymous changes, and the latter for synonymous\n";
      exit(1);
    }
  unsigned argn=1;
  const int nsam = atoi(argv[argn++]);
  unsigned howmany = atoi(argv[argn++]);
  const double thetas = atof(argv[argn++]);
  const double thetar = atof(argv[argn++]);
...
*/
      SimData d = infinite_sites_sim_data(boost::bind(gsl_ran_poisson,r,_1),
					  ...
					  nsites,sample_history,thetar);
      cout << d << endl;
      d = infinite_sites_sim_data(boost::bind(gsl_ran_poisson,r,_1),
				  boost::bind(gsl_ran_flat,r,_1,_2),
				  nsites,sample_history,thetas);
      cout << d << endl;
    }    
}
