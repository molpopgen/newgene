...
int main(int argc, char **argv)
{
  ...
  const double econv_min = atof(argv[argn++]);    //ectopic conversion rate, i.e. 4Nc, where c is the rate per generation
  const double econv_max = atof(argv[argn++]);    //ectopic conversion rate, i.e. 4Nc, where c is the rate per generation
  ...
  while(howmany--)
    {
      double econv = gsl_ran_flat(r,econv_min,econv_max);
      const double littleec = econv/double(nsites-1);
      cerr << econv << '\n';
      vector<chromosome> sample(isample);
      vector<pair<int,int> > paired(__paired);
      arg sample_history(1,imarg);
      NSAM = 2*nsam;
      NLINKEDPAIRS=nsam;
